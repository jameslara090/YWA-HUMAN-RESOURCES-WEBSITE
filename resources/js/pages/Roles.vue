        ...
        >
            ...
            <template v-slot:item.actions="{ item }">
                <div :style="`${can.edit && can.remove ? 'display:flex;' : ''}width:60px;`">
                    ...
                    <v-icon v-if="can.edit" small class="mr-2" @click="editRole(item)"> mdi-pencil </v-icon>
                    <v-icon v-if="can.remove" small @click="confirm_dialog_item=item" :disabled="auth.rolesArray.includes(item.name)"> {{ auth.rolesArray.includes(item.name) ? 'mdi-cancel' : 'mdi-delete' }} </v-icon>
                </div>
            </template>
